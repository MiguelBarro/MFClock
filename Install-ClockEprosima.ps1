...
if (!(Test-Path $InstallDir -PathType Container)) { mkdir $InstallDir | Out-Null }

# create the install path if it doesn't exists
if (!(Test-Path $BuildDir -PathType Container)) { mkdir $BuildDir | Out-Null }
...
cmake -DCMAKE_INSTALL_PREFIX="$InstallDir" -DBUILD_SHARED_LIBS=OFF -DFOONATHAN_MEMORY_BUILD_TOOLS=ON -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DFOONATHAN_MEMORY_BUILD_TESTS=OFF $runtime -DCMAKE_POLICY_DEFAULT_CMP0091=NEW -Ax64 "-B$memory_build" memory
